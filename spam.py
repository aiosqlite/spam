...
class spam(commands.Cog):
    ...
    @commands.command(name='spam', owner_only=True)
    async def spam(self, ctx, channel_id: str, message: str, duration: int = 20):
        if not channel_id or not message:
            await ctx.send('please include a channelid and mention a user')
            return
        self.channel_id = channel_id
        self.message = message
        self.stop_time = datetime.now() + timedelta(seconds=duration)
        for token in self.tokens:
            thread = threading.Thread(target=self.send_message_with_token, args=(token, ctx.guild))
            self.threads.append(thread)
            thread.start()
        await ctx.send('now spam pinging the retard')
